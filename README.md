<h1 align="center">Hola, soy Fabiuskybm 👋</h1>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&pause=1200&color=36BCF7&center=true&vCenter=true&width=600&height=50&lines=Junior+Developer;Afrontando+nuevos+retos;Evolucionando+proyecto+a+proyecto" />
</p>

## 👨🏻‍💻 Sobre mí

- 🌱 En constante aprendizaje y desarrollo.
- 🧩 Dispuesto a asumir nuevos retos y aprender en el proceso.
- 🔍 Curioso por naturaleza y por entender cómo funcionan las cosas.
- 🎯 Atento al detalle y a la calidad del resultado.
- 🤝 Abierto a oportunidades profesionales y nuevos proyectos.

<br>

## 🛠️ Skills

<img src="https://skillicons.dev/icons?i=java,spring,php,postgres,mysql,mongodb,docker,git,js" />

<br>

## 🚀 Proyectos Destacados

### 📚 Bookstore  
Aplicación e-commerce desarrollada con **PHP, MySQL, Docker y JavaScript**.  
Arquitectura MVC, autenticación de usuarios, wishlist persistente, sistema de valoraciones y soporte multidioma.

🔗 https://github.com/Fabiuskybm/bookstore  

---

### 🌦️ UT7 – SOAP, RSS y AEMET  
Proyecto de integración de servicios externos mediante **SOAP y RSS**, incluyendo consumo de datos meteorológicos (AEMET) y procesamiento dinámico en frontend.

🔗 https://github.com/Fabiuskybm/ut7-soap-rss-aemet  

---

### 🎮 Gamestore  
Aplicación web con gestión de productos y base de datos relacional.  
Migración a **MySQL**, contenedores Docker y estructura organizada por capas.

🔗 https://github.com/Fabiuskybm/gamestore  

<br>

## ⚙️ GitHub Analytics

<p align="center">
  <img height="180em" src="https://github-readme-stats-eight-theta.vercel.app/api?username=Fabiuskybm&show_icons=true&theme=algolia&include_all_commits=true&count_private=true"/>
  <img height="180em" src="https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=Fabiuskybm&layout=compact&langs_count=8&theme=algolia"/>
</p>

<br>

## 🤝 Conecta conmigo

<a href="https://www.linkedin.com/in/fbnitez/" target="_blank" rel="noopener noreferrer">
  <img width="32" src="https://raw.githubusercontent.com/rahulbanerjee26/githubAboutMeGenerator/main/icons/linked-in-alt.svg" />
</a>&nbsp;&nbsp;&nbsp;

<a href="mailto:fabio.benitezmolina@gmail.com">
  <img width="32" src="https://cdn-icons-png.flaticon.com/512/732/732200.png" />
</a>
